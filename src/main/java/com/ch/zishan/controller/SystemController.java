...
public class SystemController {
    ...
    public Result<CardGroup> shareList(@PathVariable String code) {if (StrUtil.isBlank(code)) {
    ...
    public Result<String> collect(@PathVariable Long cardGroupId) {
        CardGroup group = cardGroupService.getCardGroupById(cardGroupId);
        ...
        if (SysUtils.checkUser(group.getCreateUser(), BaseContext.get()) ||
                collectService.getOne(new QueryWrapper<Collect>()
                        .eq("user_id", BaseContext.get())
                        .eq("card_group_id", cardGroupId)) != null) {
            return Result.error("404", "已经收藏过了");
        }

        Collect collect = new Collect();
        collect.setUserId(BaseContext.get());
        collect.setCardGroupId(cardGroupId);
        collect.setCollectDate(TimeUtils.getCurrentTimeStamp());
        collectService.addCollect(collect);
        return Result.success("收藏成功");
    }

    @PostMapping("/collect/id/{cardGroupId}")
    public Result<String> collectById(@PathVariable Long cardGroupId) {
        return collect(cardGroupId);
    }

    @PostMapping("/collect/code/{shareCode}")
    public Result<String> collectByCode(@PathVariable String shareCode) {
        ...
        return collect(share.getCardGroupId());
    }
